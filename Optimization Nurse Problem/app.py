...
def schedule(num_nurses,shift_requests,c01,c02,c1,c2,c3,c4,Nurses):
    ...
    # Each shift is assigned to how many nurses? .
    if c01 == 'atleast':
        for d in all_days:
            for s in all_shifts:
    ...
    sch = []
    shift_name = {0:"Morning",1:"Afternoon",2:"Night"}
    ...
    num = {'Morning': 1, 'Afternoon': 2, 'Night':3}
    schedule_list_sorted = []
    for i in range(len(sch)):
        temp = [ num[ j[1] ] for j in sch[i]]
        zipped_lists = zip(temp,sch[i])
        sorted_zipped_lists = sorted(zipped_lists)
        temp = [element for _, element in sorted_zipped_lists]
        schedule_list_sorted.append(temp)

    return schedule_list_sorted
@app.route("/")
def main():
...
def submit():
    if request.method == 'POST':
        day1 = request.form.get('mon')
        day2 = request.form.get('tue')
        day3 = request.form.get('wed')
        day4 = request.form.get('thu')
        day5 = request.form.get('fri')
        day6 = request.form.get('sat')
        day7 = request.form.get('sun')
        ...
        if not request.form['nursename']:
            ...
            db.session.add(nur)
            db.session.commit()
            return redirect(url_for("listNurses",success=1))
...
def submit2():
    if request.method == 'POST':
        ...
        c01=request.form.get('c01')
        c02=request.form.get('c02')
        ...
        sc = schedule(len(Nurses),shift_requests,c01,c02,c1,c2,c3,c4,Nurses)
        day=['','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']
        if len(sc)==0:
            return render_template("week.html",sch=sc,days=day,optimal=1)
        return render_template("week.html",sch=sc,days=day)
    return redirect(url_for('main'))


@app.route("/listNurses/<int:success>/")

def listNurses(success=0):

    Nurses = nurses.query.all()

    return render_template("listNurses.html", nurses=Nurses,success=success)



@app.route("/nurseDetails/<int:nurse_id>/")

def nurseDetails(nurse_id):

    nurse = nurses.query.get(nurse_id)

    return render_template("nurseDetails.html", nurse=nurse)
